...
const nottes = require('../models/notes-collection.js');
...
class Note {
    ...
    async add(opts) {
        ...
        let save = await nottes.create(opts)
        return save;
    }
    async getAllNotes(){
        console.log('>>>>>>>>>>getAllNotes');
        let allNotes = await nottes.get({});
        return allNotes;
    }
    async getAllNotesByCat(cat){
        // console.log('>>>>>>getAllNotesByCat',cat);
        let allNotes = await nottes.get({catDesc : cat});
        return allNotes;
    }
    async deleteNote(id){
        // console.log('>>>>>>getAllNotesByCat',cat);
        let allNotes = await nottes.delete({_id : id});
        return allNotes;
    }
    // async updateNote(id,data){
    //     // console.log('>>>>>>getAllNotesByCat',cat);
    //     let allNotes = await nottes.update({_id : id},{data : data});
    //     return allNotes;
    // }
    async execute(opts) {
        if (opts) {
            ...
            switch (opts.method) {
                ...
                //             let result =  await this.deleteNote(testMsg);
                //             if(result){
                //                 console.log(`the note ${testMsg} has beed updated`);
                //             }else{
                //                 console.log('the note is not exist');
                //             }
                //         }
